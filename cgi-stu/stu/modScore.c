...
int cgiMain()
{
	...
	status = cgiFormString("class",  class, 10);
	if (status != cgiFormSuccess)
	{
		fprintf(cgiOut, "get class error!\n");
		return 1;
	}
	status = cgiFormString("score",  score, 10);
	if (status != cgiFormSuccess)
	{
	...
	sprintf(sql, "update score set class='%s' ,score= %d,flag=%d where sno = %d ",class,atoi(score),atoi(flag),atoi(sno));
